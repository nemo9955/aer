...
def init(pparser):
    ...
    moptions_gr.add_argument('-me', "--make-e", dest="make_e",
                             help='Add -e argument to make', action="store_true")
    moptions_gr.add_argument('-md', "--make-d", dest="make_d",
                             help='Add -d argument to make', action="store_true")
    nodes_pars.add_argument("-n", '--show-serial', dest="show_serial",
                            help='Terminal will show board serial', action="store_true")
    nodes_pars.add_argument("-N", "--serial-baud", dest="serial_baud", metavar="baud",
                            help='Select serial terminal baud rate', default=odb.var.serial_baud)
    moptions_gr.add_argument("-c", "--clear-builds", dest="clear_builds",
                             help='Clear the builds binaries', action="store_true")

    make_action.add_argument("-e", "--erase-flash", dest="erase_flash",
                             help='Erase flash of the ESP', action="store_true")
    make_action.add_argument("-d", "--flash-filesystem", dest="flash_filesystem",
                             help='Flash root directory system', action="store_true")
    make_action.add_argument("-o", "--http-ota-update", dest="http_ota_update",
    ...
    search_qu.add_argument('-D', "--in-filesystem", dest="in_filesystem", metavar="search",
                           help='String the name of the filesystem root folder must contain', default="")

    board_detect.add_argument('-P', "--boards-from-ports", dest="boards_from_ports",
                              help='Iterate thru all the valid ports to find vallid boards', action="store_true")
    board_detect.add_argument('-M', "--boards-from-mdns", dest="boards_from_mdns",
